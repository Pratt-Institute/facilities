...
class SectionsController extends \yii\web\Controller
{
    ...
    public function actionLookup() {
		...
		foreach($this->result as $key => $val) {
			...
			$this->hall = $exp_rm[0];
			$this->buildingId = Yii::$app->mycomponent->fetchBuildingsByCode($this->hall);
	...
	public function actionSearch()
	{
		...
		if ($rowCount > 0) {
			...
			foreach($this->result as $key => $val) {
				...
				$out[$srcInt]['type'] = 'Course';

				$out[$srcInt]['id']				= '';
				$out[$srcInt]['bldg_id']		= $this->buildingId;
				$out[$srcInt]['floor_id']		= '';
				$out[$srcInt]['bldg_name']		= '';
				$out[$srcInt]['bldg_abbre']		= '';

				$out[$srcInt]['room_no']		= $val['room'];
				$out[$srcInt]['room_name']		= $val['room'];
				$out[$srcInt]['on_off_campus']	= $on_off;

				$out[$srcInt]['school']			= '';
				$out[$srcInt]['department']		= '';

				$out[$srcInt]['title']			= $val['title'];
				$out[$srcInt]['phone']			= '';
				$out[$srcInt]['lname']			= '';
				$out[$srcInt]['fname']			= '';
				$out[$srcInt]['mail']			= '';

				$out[$srcInt]['professor']		= $val['instructor_last'];
				$out[$srcInt]['course']			= $course;
				$out[$srcInt]['times']			= $val['time'];

				$out[$srcInt]['sculpture']		= '';
				$out[$srcInt]['artist']			= '';
				$out[$srcInt]['latitude']		= $coordsExp[0];
			...
			$out[$srcInt]['type']		= 'Course';
			$out[$srcInt]['bldg_id']	= 'No Matches';
			$out[$srcInt]['message']	= 'No Matches Found';

		}

		if (count($out) < 1) {
			$out[$this->srcInt]['type']		= 'Course';
